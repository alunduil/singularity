        ...
        * The resolver entries don't already exist
        ...
        with open(self.resolvconf_path, "r") as resolvconf:
            for line in resolvconf:
                match = re.search(r"nameserver.*(?P<ip>(?:\d{1,3}\.){3}\d{1,3})", line)
                if not match:
                    continue
                existing_resolvers.add(match.group("ip"))
        if existing_resolvers >= set([ resolver[0] for resolver in configuration["resolvers"] ])
            return False

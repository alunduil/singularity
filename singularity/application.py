...
class SingularityApplication(object): # pylint: disable=R0903
    def __init__(self):
        ...
        dhl.setFormatter(logging.Formatter("%(asctime) %(levelno) singularity %(levelname)s => %(name)s: %(pathname)s:%(lineno)d in %(funcName)s: %(message)s", '%b %e %H:%M:%S')) # pylint: disable=C0301
        nhl.setFormatter(logging.Formatter("%(asctime) %(levelno) singularity %(levelname)s => %(name)s: %(message)s", '%b %e %H:%M:%S')) # pylint: disable=C0301
